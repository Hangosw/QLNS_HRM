...
<body>
    ...
    <div class="container">
        ...
        <div>
            <span class="text-bold">Chúng tôi, một bên là Ông/Bà:</span> NGUYỄN VÀ VĂN B <span style="float: right;">Quốc tịch: Việt Nam</span><br>
        ...
        <br>
        <div>
            <span class="text-bold">Và một bên là Ông/Bà:</span> <span class="text-bold" style="text-transform: uppercase;">{{ $hopDong->nhanVien->Ten ?? '...' }}</span> <span style="float: right;">Quốc tịch: Việt Nam</span><br>
            <span class="text-bold">Sinh ngày:</span> {{ $hopDong->nhanVien->NgaySinh ? \Carbon\Carbon::parse($hopDong->nhanVien->NgaySinh)->format('d/m/Y') : '.../.../......' }} &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
            <span class="text-bold">Tại:</span> {{ $hopDong->nhanVien->NoiSinh ?? '...................' }}<br>
            <span class="text-bold">Nghề nghiệp:</span> Nhân viên<br>
            <span class="text-bold">Địa chỉ thường trú:</span> {{ $hopDong->nhanVien->ThuongTru ?? '...................' }}<br>
            <span class="text-bold">Số CCCD/CMND:</span> {{ $hopDong->nhanVien->CCCD ?? '...................' }} &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <span class="text-bold">Cấp ngày:</span> {{ $hopDong->nhanVien->NgayCapCCCD ? \Carbon\Carbon::parse($hopDong->nhanVien->NgayCapCCCD)->format('d/m/Y') : '.../.../......' }} &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <span class="text-bold">Nơi cấp:</span> {{ $hopDong->nhanVien->NoiCapCCCD ?? '...................' }}<br>
        </div>
        
        <p class="text-italic" style="margin-top: 15px;">Thoả thuận ký kết hợp đồng lao động và cam kết làm đúng những điều khoản sau đây:</p>

        <h2>Điều 1: THỜI HẠN VÀ CÔNG VIỆC HỢP ĐỒNG</h2>
        <ul>
            <li><span class="text-bold">Loại hợp đồng lao động:</span> {{ $hopDong->loaiHopDong->TenLoai ?? ($hopDong->Loai ?? '...................') }}</li>
            <li><span class="text-bold">Từ ngày:</span> {{ $hopDong->NgayBatDau ? \Carbon\Carbon::parse($hopDong->NgayBatDau)->format('d/m/Y') : '.../.../......' }} 
                @if($hopDong->NgayKetThuc)
                    đến ngày {{ \Carbon\Carbon::parse($hopDong->NgayKetThuc)->format('d/m/Y') }}
                @endif
            </li>
            <li><span class="text-bold">Địa điểm làm việc:</span> Số 123 Đường Công Nghệ, Phường Sáng Tạo, Quận 1, TP.HCM</li>
            <li><span class="text-bold">Chức danh chuyên môn:</span> {{ $hopDong->chucVu->Ten ?? 'Nhân viên' }}</li>
            <li><span class="text-bold">Công việc phải làm:</span> Liên quan đến chuyên môn và những công việc khác do Giám đốc (hoặc cá nhân được Giám đốc ủy quyền) phân công theo quy định của Pháp luật.</li>
        </ul>
        ...
        <ul>
            ...
            <li><span class="text-bold">Mức lương chính:</span> {{ number_format($hopDong->MucLuong ?? 0, 0, ',', '.') }} VNĐ/tháng (tại thời điểm ký hợp đồng).</li>
        ...
        <ul>
            ...
            <li>Bồi thường vi phạm: Theo qui định của Công ty và Luật lao động. Doanh nghiệp yêu cầu Người lao động chịu bồi thường thiệt hại gây ra do vi phạm quy định điều khoản.</li>
        </ul>
        ...
        <ul>
            ...
            <li>Có quyền điều hành người lao động hoàn thành công việc theo hợp đồng, bố trí điều chuyển công việc hợp lý.</li>
            <li>Đơn phương chấm dứt hợp đồng nếu người lao động vi phạm nghiêm trọng nội quy, không hoàn thành chỉ tiêu, hoặc có hành vi gian lận.</li>
        </ul>

        <h2>Điều 5: ĐIỀU KHOẢN THI HÀNH</h2>
        <ul>
            <li>Những vấn đề chưa ghi trong hợp đồng này sẽ áp dụng theo nội quy lao động công ty hoặc quy định pháp luật.</li>
            <li>Hợp đồng được làm thành 02 bản có giá trị pháp lý như nhau, mỗi bên giữ 01 bản.</li>
        </ul>

        <div style="text-align: right; margin-top: 20px;">
            <span class="text-italic">Hợp đồng này lập tại văn phòng công ty ngày {{ date('d') }} tháng {{ date('m') }} năm {{ date('Y') }}.</span>
        </div>
